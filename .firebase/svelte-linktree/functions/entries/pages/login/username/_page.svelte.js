...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(AuthCheck, "AuthCheck").$$render($$result, {}, {}, {
    default: () => {
      return `${$userData?.username ? `<p class="text-lg">Your username is <span class="text-success font-bold">@${escape($userData.username)}</span></p> <p class="text-sm mt-1 mb-2" data-svelte-h="svelte-dzoiqw">(Usernames cannot be changed)</p> <a class="btn btn-primary" href="/login/photo" data-svelte-h="svelte-vogdig">Upload Profile Image</a>` : `<form class="w-2/5"><input type="text" placeholder="Username" class="${[
        "input w-full",
